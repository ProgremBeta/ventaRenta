...
export const obtenerPagosDeudas = async () => {
  return await pool.query('SELECT * FROM pagos_deudas');
};

export const obtenerPagoDeudaPorId = async (id) => {
  return await pool.query('SELECT * FROM pagos_deudas WHERE id =$1', [id]);
};
...
export const eliminarPagoDeuda = async (id) => {
  return await pool.query('DELETE FROM pagos_deudas WHERE id=$1 RETURNING *', [id]);
};